...
def company_edit_product(product_id):
    ...
    return render_template(
        ...
        product=product,
        product_id=product_id,
        current_page=current_page,category=category,sub_category=sub_category
    )
...
def save_product(product_id):
    ...
    return render_template(
        "edit_product.html", product_id=product_id, current_page=current_page,category=category,sub_category=sub_category
    )
...
def company_add_product():
    ...
    return render_template("add_product.html", current_page=current_page,category=category,sub_category=sub_category)
...
def company_view_orders():
    ...
    #     return " No more products available"
    return render_template(
        "company_order_view.html", orders=orders, current_page=current_page
    )
...
def company_order(order_id):
    ...
    if "logged_in" not in session or not session["logged_in"]:
        ...
        cursor.close()
        return render_template(
            "view_company_order.html", order=order, current_page=current_page
        )
...
def company_update_order_status():
    ...
    if "logged_in" not in session or not session["logged_in"]:
        ...
        return redirect(url_for("company_home"), current_page=current_page)
